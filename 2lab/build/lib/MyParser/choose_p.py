...
class Serializer:
    ...
    def serialize(obj, format):
        ...
        if expr_type == "dump":
            ...
            serializer.dump(serializer, file_path, MyParser.my_p.pack(obj))
        elif expr_type == "dumps":
            loaded_string = serializer.dumps(serializer, MyParser.my_p.pack(obj))
            print(loaded_string)
        elif expr_type == "load":
            file_path = get_file_path()
            loaded_obj = MyParser.my_p.unpack(serializer.load(serializer, file_path))
            loaded_obj.greetings()
            print(loaded_obj)
        elif expr_type == "loads":
            obj_string = get_obj_string()
            loaded_obj = MyParser.my_p.unpack(serializer.loads(serializer, obj_string))
            print(loaded_obj)
